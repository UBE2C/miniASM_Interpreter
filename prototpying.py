...
def float_multiplier(multiplicand: float | int, multiplier: float | int, precision: int = 64) -> float:
    ...
    if num_2_exp.count(1) == 0 and (multiplicand_mantissa.count(1) != 0):
...
def sub_biased_exponents(exponent_1: list[int], exponent_2: list[int], intermediate_len: int) -> list[int]:
...
def fp_bytearray_to_bitlist(fp_number: bytearray | bytes, var_type: str, bit_length: int) -> list[int]:
    """
    ...
    """
    if not isinstance(fp_number, (bytearray, bytes)) and var_type == "float":
    ...
    fp_num: bytearray | bytes = fp_number[::-1]
...
#NOTE: this is the working split exponent calculation in the FPU which I will swap with the unified add_sub function
...
#NOTE: original working multiply floats function! (the edge case handling is not perfect in either of them and need revision)
...
#Unified floating point multiplication and division method (not complete)
    def multiply_divide_floats(self, operation: str, first_opernad: bytearray | bytes, second_operand: bytearray | bytes, first_opernad_type: str, second_opernad_type: str, precision: int = 64) -> float:
        """
        Multiply two floating-point numbers using IEEE 754 compliant binary arithmetic with comprehensive subnormal support.
        ...
        #Argument type checks
        if not (first_opernad_type == "float" or first_opernad_type == "int") or not (second_operand_type == "float" or second_operand_type == "int"):
            raise TypeError(f"multiply_divide_floats: the arguments dividend and divisor must be of type float or int, {first_opernad_type} and {second_operand_type} were provided.")
        ...
        if operation != "mult" or operation != "div":
        ...
        n1_bit_lst: list[int] = self.fp_bytearray_to_bitlist(number = first_opernad, var_type = first_opernad_type, bit_length = 64, bias = 1023)
        n2_bit_lst: list[int] = self.fp_bytearray_to_bitlist(number = second_operand, var_type = second_opernad_type, bit_length = 64, bias = 1023)
        ...
        #Edge case handling 
        if operation == "mult": #for multiplication
            #Weird edge case check for 0 * +/- Inf which should produce a NaN
            if (n1_bit_lst.count(1) == 0 or n2_bit_lst.count(1) == 0) and ((n1_bit_lst[1 : exp_len + 1].count(0) == 0 and n1_bit_lst[exp_len + 1 : mant_len].count(1) == 0)
                                                                        or (n2_bit_lst[1 : exp_len + 1].count(0) == 0 and n2_bit_lst[exp_len + 1 : mant_len].count(1) == 0)):
                final_exponent: str = "1" * exp_len #exponent must be all 1s
...
#NOTE: the original working split multiply and divide functions
#External method
